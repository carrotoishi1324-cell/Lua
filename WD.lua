...
local function GetTarget()
    ...
    for _, plr in Players:GetPlayers() do
        ...
        if Aimbot.Airshot and plr.Character:FindFirstChild("Humanoid") and not plr.Character.Humanoid.FloorMaterial then
            pos = pos + Vector3.new(0, Aimbot.AirshotOffset, 0)
        end
        ...
        if Aimbot.SilentAim and math.random(1, 100) > Aimbot.SilentAimHitChance then continue end
...
local function Triggerbot()
    ...
    if tick() - lastTriggerTime < Aimbot.TriggerbotDelay then return end
...
-- Create logo with decal
local LogoDecalId = "10110319532"
local LogoGui = Instance.new("ScreenGui")
LogoGui.Name = "WD_Logo"
...
LogoFrame.Position = UDim2.new(1, -120, 1, -120)
...
LogoImage.Image = "rbxassetid://" .. LogoDecalId
LogoImage.Parent = LogoFrame

Library:Watermark("WD | V 2.0")
local Window = Library:Window("WD", true, LogoDecalId)
...
EspSettingsGroup:AddSlider({ Title = "Text Size", Min = 8, Max = 24, Default = 13, Callback = function(V) ESP_Settings.TextSize = V end })
EspSettingsGroup:AddDropdown({ Title = "Tracer Origin", Values = {"Bottom", "Center", "Mouse"}, Default = "Bottom", Callback = function(V) ESP_Settings.Tracer.Origin = V end })
...
ChamsGroup:AddSlider({ Title = "Fill Transparency", Min = 0, Max = 1, Default = 0.5, Increment = 0.1, Callback = function(V) ESP_Settings.Chams.FillTransparency = V end })
ChamsGroup:AddSlider({ Title = "Outline Transparency", Min = 0, Max = 1, Default = 0, Increment = 0.1, Callback = function(V) ESP_Settings.Chams.OutlineTransparency = V end })
...
local AdvancedPage = AimbotTab:SubTab("Advanced")

AdvancedPage:Groupbox("Triggerbot", "Left"):AddToggle({
    Title = "Enable Triggerbot",
    Default = false,
    Callback = function(v) Aimbot.Triggerbot = v end
})

AdvancedPage:Groupbox("Triggerbot", "Left"):AddSlider({
    ...
    Increment = 0.01,
    Suffix = " seconds",
    Callback = function(v) Aimbot.TriggerbotDelay = v end
})

AdvancedPage:Groupbox("Silent Aim", "Right"):AddToggle({
    Title = "Enable Silent Aim",
    Default = false,
    Callback = function(v) Aimbot.SilentAim = v end
})

AdvancedPage:Groupbox("Silent Aim", "Right"):AddSlider({
    Title = "Hit Chance %",
    Min = 1,
    Max = 100,
    Default = 90,
    Callback = function(v) Aimbot.SilentAimHitChance = v end
})

AdvancedPage:Groupbox("Airshot", "Left"):AddToggle({
    Title = "Enable Airshot",
    Default = false,
    Callback = function(v) Aimbot.Airshot = v end
})

AdvancedPage:Groupbox("Airshot", "Left"):AddSlider({
    Title = "Airshot Offset",
    Min = -50,
    Max = 50,
    Default = 0,
    Callback = function(v) Aimbot.AirshotOffset = v end
})
...
UISettings:AddButton({
    ...
    Callback = function()
        ...
        local logo = game:GetService("CoreGui"):FindFirstChild("WD_Logo")
        local fov = game:GetService("CoreGui"):FindFirstChild("WD_FOV")
...
local MiscPage = MiscTab:SubTab("Utilities")

MiscPage:Groupbox("Player", "Left"):AddSlider({
    Title = "WalkSpeed",
    Min = 16,
    Max = 200,
    Default = 16,
...
MiscPage:Groupbox("Player", "Left"):AddToggle({
    Title = "Noclip",
    Default = false,
    Callback = function(v)
        if v then
...
MiscPage:Groupbox("Player", "Right"):AddToggle({
    Title = "Infinite Jump",
    Default = false,
    Callback = function(v)
        if v then
...
MiscPage:Groupbox("Camera", "Left"):AddSlider({
    Title = "Field of View",
    Min = 70,
    Max = 120,
    Default = 70,
...
MiscPage:Groupbox("Game", "Right"):AddButton({
    Title = "Rejoin Server",
    Callback = function()
...
Library:Notify("WD Script", "Successfully loaded! Version 2.0", 5)
